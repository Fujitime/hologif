...
export function DisplayHoloData() {
  ...
  let currentIndex = 1; // Start with member ID 1
  ...
  if (infoContainer) {
    ...
    // Function to get data from Local Storage or fetch from API and save to Local Storage
    ...
    const displayMemberData = (id: number) => {
      Promise.all([
        ...
        .catch((error) => {
          console.error(error);
          // The error handling is already done inside the getDataFromCacheOrFetch function, so we don't need to do anything here.
        });
    };
    ...
    // Show the first member on initial page load
    displayMemberData(currentIndex);
